...
class CapsuleNet(nn.Module):
    ...
    def __init__(self, input_size, classes, routings):
        ...
        self.digitcaps = DenseCapsule(in_num_caps=118*118*256, in_dim_caps=8,
...
class cas_res_Net(nn.Module):
    def __init__(self):
        super(cas_res_Net, self).__init__()
        self.res=ResNet.resnet34(num_classes=1000, include_top=True)
        self.cas=CapsuleNet(input_size=[3,244,244],classes=3,routings=3)
    def forward(self,x):
        out1 = self.cas(x)
        out1 = nn.functional.softmax(out1)
        out2 = self.res(x)
        return out1+out2